...
import com.infore.csep.pojo.mapper.FactsewahisMapper;
...
@Service
@Slf4j
public class FactsewahishourServiceImpl extends ServiceImpl<FactsewahishourMapper, Factsewahishour> implements IFactsewahishourService {

    //数据源
    @Autowired
    private FactsewahisMapper factsewahisMapper;
    ...
    @Override
    public Factsewahis findOneByFactNumAndEqumNum(Integer factNum, String equmNum, Date beginTime, Date endTime) {

        Wrapper<Factsewahis> wrapper = new EntityWrapper<Factsewahis>()
                .eq("factNum", factNum).eq("equmNum", equmNum)
                .ge("revTime", beginTime).lt("revTime", endTime)
                .orderBy("revTime", false)
                .last("limit 1");

        List<Factsewahis> factsewahis = factsewahisMapper.selectList(wrapper);
    ...
    public void genData(Date beginTime, Date endTime) {
        ...
        List<Factsewahishour> factsewahishours = factsewahishourMapper.findByTime(beginTime, endTime);

        for (Factsewahishour item : factsewahishours) {

            //2.根据厂站编码和设备编码，在时间范围内查询最新的一条记录
            Factsewahis factleachhis = findOneByFactNumAndEqumNum(item.getFactNum(),
        ...
        for (Factsewahishour factDay : factsewahishours) {
            log.info("Factsewahishour: {}", factDay);
        }
    ...
    @Override
    public List<Factsewahishour> findByFactNumAndEqumNum(Integer factNum, String equmNum, Date beginTime, Date endTime) {
