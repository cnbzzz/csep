...
public class GpsSewaCtrl {
     ...
     * @return
     */
    public List<Gpssewa> findByFactNumAndEqumNum(String factNum, String equmNum) {
        ...
        List<Gpssewa> realTimeRec = gpssewaService.selectList(wrapper);
    ...
    public ResultVO realtime(@RequestBody @Valid RealTimeReqVO reqVO, BindingResult br) {
        ...
        //参数合法性较验
        log.debug("realtime: {}", reqVO);
        CheckParamUtil.check(br);

        List<Gpssewa> realTimeRec = findByFactNumAndEqumNum(reqVO.getFactNum(), reqVO.getEqumNum());
        //log.debug("date: {}", DateFormatUtils.format(realTimeRec.get(0).getRevTime(), "yyyy-MM-dd HH:mm:ss"));
    ...
    public ResultVO openStatus(@RequestBody @Valid RealTimeReqVO reqVO, BindingResult br) {
        ...
        List<Gpssewatime> gpssewatimes = gpssewatimeService.findByFactNumAndEqumNum(
                reqVO.getFactNum(),
                reqVO.getEqumNum());
